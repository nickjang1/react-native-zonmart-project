...
import { Container, Header, Icon, InputGroup, Input, Content, Picker, Title, Button } from 'native-base';
import theme from '../../themes/base-theme';
import styles from './styles';
import Global from '../../Global';
import { checkoutBilling, setLanguage } from '../../actions/Creators';
...
class Checkout2 extends Component {  // eslint-disable-line
  static propTypes = {
  ...
  onCheckOut() {
    if (this.state.firstname === null || this.state.lastname === null ||
      ...
      this.replaceAt('checkout3');
    }
  }
  onForward() {
    this.replaceAt('checkout1');
  }
  replaceAt(route) {
    this.props.replaceAt('checkout2', { key: route }, 'global');
  }
  render() { // eslint-disable-line class-methods-use-this
    ...
    const provices = this.props.provinceReducer.province.map(function (item) {
      return (
        <Item label={item.name} textStyle={{ color: theme.darkGray }} value={item.id} key={item.id} />
      );
    });
    return (
          ...
          <View style={[styles.headerView, { justifyContent: 'space-between' }]}>
            ...
            >
              ...
              <Text style={styles.backText}>
                ...
                ({Global.Languages.BillingAddr[this.props.language]})
              </Text>
            </TouchableOpacity>
...
function mapStateToProps(state) {
  ...
  return { language, billingReducer, provinceReducer };
}

export default connect(mapStateToProps, bindAction)(Checkout2);
